...
AI 旅行规划后端服务。基于 LLM function-calling + SSE 实时流，整合高德天气、飞猪旅行数据，提供结构化行程规划和自由对话两种模式。
...
npm install
npm run setup           # 安装飞猪 CLI 和 skills
npm run dev             # tsx watch，默认 :3333
```

> 💡 `npm run dev` 和 `npm start` 会自动在启动前运行 `setup` 脚本，确保 flyai CLI 环境就绪。
...
| `FLYAI_API_KEY` | - | 飞猪旅行 API Key（flyai CLI） |
| `PORT` | - | 服务端口，默认 `3333` |
...
检查飞猪 API Key 和 CLI 状态。

### `POST /api/fliggy/token`
保存/更新飞猪 API Key。
...
| `search_fliggy_travel` | 飞猪旅行搜索（酒店/门票/机票/火车票/度假） |
| `check_fliggy_token` | 检查飞猪 API Key 配置状态 |
| `save_fliggy_token` | 保存/同步飞猪 API Key 到 flyai CLI 配置 |

## Agent 执行流程

- 纯 JSON 输出（TripPlan 结构）
- **短期记忆**：每 session 保留最近 4 轮对话，跨请求记住上下文（城市/偏好/已讨论内容）
- Step 0 分析需求 → Step 1 查天气 → Step 2 调飞猪 → Step 3 生成行程
- 信息不足时输出追问，下一条消息继续对话（上下文保持）
- 禁止预判具体景点/酒店/餐厅名称（让飞猪推荐）
...
src/
  ...
  routes/
    ...
    fliggy.ts           # /api/fliggy/*
  services/
    weather.ts          # 高德天气（城市名或 adcode，含 fallback）
    routes.ts           # 路线计算
    fliggy-travel.ts    # 飞猪 CLI（flyai）封装
  skills/
    fliggy-travel.md    # 飞猪旅行 Skill 定义
  scripts/
    setup-flyai.sh      # 飞猪环境初始化脚本
```
...
| 命令 | 说明 |
|------|------|
| `npm run setup` | 安装 flyai CLI 并配置 API Key |
| `npm run dev` | 开发模式（先 setup 再 tsx watch） |
| `npm run build` | 编译 TypeScript → dist/ |
| `npm start` | 生产启动（先 setup 再 node） |
