...
Token 读取优先级：**环境变量 `MEITUAN_API_TOKEN`** > `~/.config/meituan-travel/config.json`

服务启动时会自动将 `.env` 中的 Token 同步到 `~/.config/meituan-travel/config.json`，确保 `mttravel` CLI 可用。

调用 `check_meituan_token` 工具检查状态：
- ✅ 如果已配置 → 直接进入 Step 4
- ❌ 如果未配置 → 进入 Step 2
...
> ```
> MEITUAN_API_TOKEN=<你的Token>
> ```
>
> **方式二：** 直接将 Token 字符串发给我，我会帮您保存。
>
> Token 获取方式：前往美团开发者中心**入驻个人开发者**并创建 Token：
...
如果用户通过对话发送了 Token，调用 `save_meituan_token` 工具保存。

保存成功后告知用户：「✅ Token 已保存，正在为您查询…」（同时建议用户将 Token 添加到 `.env` 中以持久化），然后继续执行。

### Step 4 — 执行 CLI
CLI 会从 `~/.config/meituan-travel/config.json` 读取 Token（服务启动时已自动同步），直接执行：
mttravel [城市] "<query>"
...
>
> 更新方式：
> 1. 更新服务器 `.env` 文件中的 `MEITUAN_API_TOKEN`（推荐，重启后生效）
> 2. 或将新 Token 发送给我，我会更新并重新查询

收到新 Token 后，调用 `save_meituan_token` 工具保存。
